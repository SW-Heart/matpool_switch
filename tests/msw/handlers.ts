...
export const handlers = [
  ...
  http.post(`${TAURI_ENDPOINT}/get_proxy_takeover_status`, () =>
    success(getTakeoverStatus()),
  ),

  http.post(`${TAURI_ENDPOINT}/set_proxy_takeover_for_app`, async ({ request }) => {
    const { appType, enabled } = await withJson<{ appType: string; enabled: boolean }>(request);
    setTakeoverStatusForApp(appType, enabled);
    return success(true);
  }),

  http.post(`${TAURI_ENDPOINT}/is_live_takeover_active`, () => {
    const status = getTakeoverStatus();
    const active = Object.values(status).some(v => v);
  ...
  // Matpool API Mock
  http.get("https://token.matpool.com/api/pricing", () => {
    return HttpResponse.json({
      data: [
        {
          model_name: "gpt-4o",
          supported_endpoint_types: ["TEXT", "CODE"],
        },
        {
  ...
  http.get("https://token.matpool.com/v1/models", () => {
    return HttpResponse.json({
      data: [
        { id: "gpt-4o" },
  ...
  // Tauri Keychain Mock
  http.post(`${TAURI_ENDPOINT}/matpool_keychain_get`, () => {
    return success("sk-mocktoken1234567890");
  }),

  http.post(`${TAURI_ENDPOINT}/matpool_keychain_set`, () => {
    return success(true);
  }),

  http.post(`${TAURI_ENDPOINT}/matpool_keychain_clear`, () => {
    return success(true);
  }),

  http.post(`${TAURI_ENDPOINT}/get_tool_versions`, () => {
    return success([
      { name: "claude", version: "1.0.0", installed_but_broken: false },
      { name: "codex", version: "1.0.0", installed_but_broken: false },
      { name: "gemini", version: "1.0.0", installed_but_broken: false },
    ]);
  }),
];
