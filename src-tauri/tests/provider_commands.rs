...
fn settings_path(home: &Path) -> PathBuf {
    home.join(".matpool-switch").join("settings.json")
}
...
"#;
    ...
    assert_eq!(
        ...
        3,
        "official seeding should add matpool-codex and codex-official alongside imported default"
    );
    assert!(providers_after_seed.contains_key("codex-official"));
    assert!(providers_after_seed.contains_key("matpool-codex"));
...
"#;
    ...
    assert_eq!(
        ...
        2,
        "fixture should start with only codex-official and matpool-codex present"
    );
    assert!(providers_before.contains_key("codex-official"));
    assert!(providers_before.contains_key("matpool-codex"));
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = matpool_switch_lib::get_claude_settings_path();
    ...
    let db_path = std::path::Path::new(&home_dir)
        .join(".matpool-switch")
        .join("matpool-switch.db");
    assert!(
        db_path.exists(),
        "switching provider should persist to matpool-switch.db"
    );
