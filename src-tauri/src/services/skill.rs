...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.matpool-switch/skills/`
...
pub enum SkillStorageLocation {
    /// Matpool Switch 管理目录 (~/.matpool-switch/skills/)
    #[default]
    MatpoolSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.matpool-switch/skills/ 或 ~/.agents/skills/）
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::settings::get_skill_storage_location();
        let dir = match location {
            SkillStorageLocation::MatpoolSwitch => get_app_config_dir().join("skills"),
    ...
    /// 获取 Skill 卸载备份目录（~/.matpool-switch/skill-backups/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 Matpool Switch 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "matpool-switch".to_string()));
        }
    ...
    /// 从应用目录导入 Skills
    ///
    /// 将未管理的 Skills 导入到 Matpool Switch 统一管理
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "matpool-switch".to_string()));
