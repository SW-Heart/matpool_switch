...
///
/// 原因：接管模式下 `*_MODEL` 必须由 Matpool Switch 写成稳定的 Claude 角色别名，
...
pub struct ProxyService {
    ...
    app_handle: Arc<RwLock<Option<DesktopAppHandle>>>,
...
impl ProxyService {
    ...
    fn build_claude_takeover_model_fields(config: &Value) -> Vec<(&'static str, String)> {
        ...
        let opus_model = Self::claude_env_string(env, "ANTHROPIC_DEFAULT_OPUS_MODEL")
            .or(default_model)
            .or(small_fast_model);
        let custom_model = Self::claude_env_string(env, "ANTHROPIC_CUSTOM_MODEL_OPTION")
            .or_else(|| Self::claude_env_string(env, "ANTHROPIC_DEFAULT_FABLE_MODEL"));
        ...
        if let Some(custom_model) = custom_model {
            fields.push(("ANTHROPIC_CUSTOM_MODEL_OPTION", custom_model.to_string()));
            let custom_display_name =
                Self::claude_env_string(env, "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME")
                    .map(str::to_string)
                    .unwrap_or_else(|| Self::strip_claude_one_m_marker(custom_model));
            fields.push(("ANTHROPIC_CUSTOM_MODEL_OPTION_NAME", custom_display_name));
        }
        fields
    }
    ...
    fn get_current_provider_for_app(&self, app_type: &AppType) -> Result<Option<Provider>, String> {
        ...
        Ok(provider.map(|provider| {
            crate::services::matpool_inject::provider_with_injected_matpool_token(&provider)
                .unwrap_or(provider)
        }))
    }
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        self.set_takeover_for_app_inner(app_type, enabled, true)
            .await
    }

    pub async fn set_takeover_for_app_without_managing_proxy(
        &self,
        app_type: &str,
        enabled: bool,
    ) -> Result<(), String> {
        self.set_takeover_for_app_inner(app_type, enabled, false)
            .await
    }
    ...
    async fn set_takeover_for_app_inner(
        ...
        if enabled {
            // 1) 代理服务未运行则自动启动
            if manage_proxy_process && !self.is_running().await {
                self.start().await?;
            }
    ...
    fn codex_provider_config_text(provider: Option<&Provider>) -> Option<&str> {
        provider?
            .settings_config
            .get("config")
            .and_then(|value| value.as_str())
    ...
    /// Codex ignores top-level `base_url`; requests route through the active
    /// `model_provider` table. Proxy takeover must therefore materialize an
    /// active provider route before updating local proxy fields.
    fn ensure_codex_proxy_provider_route(
        toml_str: &str,
        provider: Option<&Provider>,
    ) -> Result<String, String> {
        let source = if toml_str.trim().is_empty() {
            Self::codex_provider_config_text(provider).unwrap_or(toml_str)
        } else {
            toml_str
        };
        ...
        let provider_key = doc
            ...
            .map(str::to_string)
            .unwrap_or_else(|| {
                crate::codex_config::MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID.to_string()
            });

        doc["model_provider"] = toml_edit::value(provider_key.as_str());
        ...
        if let Some(model_providers) = doc["model_providers"].as_table_mut() {
            if !model_providers.contains_key(provider_key.as_str()) {
                model_providers[provider_key.as_str()] = toml_edit::table();
            }

            if let Some(provider_table) = model_providers[provider_key.as_str()].as_table_mut() {
    ...
    fn apply_codex_proxy_toml_config_for_provider(
        ...
        let routed = Self::ensure_codex_proxy_provider_route(toml_str, provider)
            .unwrap_or_else(|_| toml_str.to_string());
        let updated = Self::update_toml_base_url(&routed, proxy_url);
        let mut updated =
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("MATPOOL_SWITCH_TEST_HOME").ok();

            env::set_var("HOME", dir.path());
            env::set_var("USERPROFILE", dir.path());
            env::set_var("MATPOOL_SWITCH_TEST_HOME", dir.path());
    ...
    impl Drop for TempHome {
        fn drop(&mut self) {
            ...
            match &self.original_test_home {
                Some(value) => env::set_var("MATPOOL_SWITCH_TEST_HOME", value),
                None => env::remove_var("MATPOOL_SWITCH_TEST_HOME"),
            }
        }
    }
    ...
    fn managed_account_claude_takeover_sources_copilot_models_from_provider() {
        ...
        let mut live_config = json!({
            "env": {
                ...
                "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME": "Stale custom"
            }
        });
        ProxyService::apply_claude_takeover_fields_for_provider(
            &mut live_config,
    ...
    fn managed_account_claude_takeover_sources_codex_models_from_provider() {
        let mut provider = Provider::with_id(
            ...
            json!({
                "env": {
                    ...
                    "ANTHROPIC_CUSTOM_MODEL_OPTION": "gpt-5.4-pro"
                }
            }),
            None,
        );
        ...
        assert_env_str(env, "ANTHROPIC_DEFAULT_FABLE_MODEL", None);
        assert_env_str(env, "ANTHROPIC_DEFAULT_FABLE_MODEL_NAME", None);
        assert_env_str(env, "ANTHROPIC_CUSTOM_MODEL_OPTION", Some("gpt-5.4-pro"));
        assert_env_str(
            env,
            "ANTHROPIC_CUSTOM_MODEL_OPTION_NAME",
            Some("gpt-5.4-pro"),
...
"#;
    ...
    #[test]
    fn apply_codex_proxy_toml_config_materializes_route_from_provider_seed() {
...
"#;
        ...
        assert_eq!(custom.get("name").and_then(|v| v.as_str()), Some("matpool"));
        assert_eq!(
        ...
        assert!(
            parsed.get("base_url").is_none(),
            "Codex should not rely on top-level base_url"
        );
    }

    #[test]
    fn apply_codex_proxy_toml_config_migrates_legacy_top_level_proxy_fields() {
...
"#
        ...
        assert!(
            parsed.get("base_url").is_none(),
            "legacy top-level base_url should be removed"
        );
        assert!(
            parsed.get("wire_api").is_none(),
            "legacy top-level wire_api should be removed"
        );
        assert_eq!(
            parsed
                .get("model_providers")
...
"#;
        ...
        assert!(
            catalog_path.exists(),
            "matpool-switch-model-catalog.json must be created on provider switch"
        );
...
"#;
    ...
    async fn codex_restore_from_backup_preserves_model_catalog_pointer() {
        ...
        // Pre-takeover Live state: config.toml points at the matpool-switch generated
        ...
        assert!(
            ...
            "restored pointer must still reference the matpool-switch generated catalog file"
        );
    }
    ...
    async fn codex_restore_from_backup_projects_inline_model_catalog() {
        ...
        assert!(
            catalog_path.exists(),
            "restore must generate the matpool-switch catalog file on disk"
        );
    ...
    async fn codex_restore_empty_auth_backup_still_projects_inline_catalog() {
        ...
        assert!(
            ...
            "empty-auth restore must generate the matpool-switch catalog file"
        );
        assert!(
