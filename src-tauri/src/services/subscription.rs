...
/// - `"codex_oauth"` + "Please re-login via matpool-switch."（matpool-switch 自管 OAuth 路径）
...
async fn refresh_gemini_token(refresh_token: &str) -> Option<String> {
    let client = crate::proxy::http_client::get();
    let client_id = std::env::var("MATPOOL_GEMINI_OAUTH_CLIENT_ID")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())?;
    let client_secret = std::env::var("MATPOOL_GEMINI_OAUTH_CLIENT_SECRET")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())?;
