...
mod tests {
    ...
    fn with_test_home<T>(test: impl FnOnce(&AppState, &Path) -> T) -> T {
        ...
        let old_test_home = std::env::var_os("MATPOOL_SWITCH_TEST_HOME");
        let old_home = std::env::var_os("HOME");
        std::env::set_var("MATPOOL_SWITCH_TEST_HOME", temp.path());
        ...
        match old_test_home {
            Some(value) => std::env::set_var("MATPOOL_SWITCH_TEST_HOME", value),
            None => std::env::remove_var("MATPOOL_SWITCH_TEST_HOME"),
        }
        match old_home {
...
"#;
    ...
    /// Regression: while proxy takeover is active, editing the current Codex
    /// provider (e.g. changing the selected model in the GUI) must rewrite the
    /// live `~/.codex/config.toml` — otherwise Codex CLI keeps reading the
    /// previous `model = "..."` and every request hits the old model.
    #[tokio::test]
    #[serial]
    async fn update_current_codex_provider_rewrites_live_config_when_proxy_takeover_active() {
...
"#
        ...
        original.category = Some("aggregator".to_string());
        db.save_provider("codex", &original)
            .expect("save codex provider");
        db.set_current_provider("codex", "matpool-codex")
            .expect("set current provider");
        crate::settings::set_current_provider(&AppType::Codex, Some("matpool-codex"))
            .expect("set local current provider");

        // Simulate an active takeover: proxy_config.enabled=true plus a live
        // backup so `is_app_taken_over` in ProviderService::update is true.
        {
            let mut config = db
        ...
        // Seed the taken-over live files: auth.json holds PROXY_MANAGED and
        // config.toml routes through the local proxy with GPT-5.5. This is
        // what `matpool takeover codex` writes.
        std::fs::create_dir_all(get_codex_auth_path().parent().unwrap())
...
"#,
        ...
        // Sanity-check that takeover is detected in the seeded live file so
        // ProviderService::update takes the proxy-aware branch.
        assert!(
            state
                .proxy_service
                .detect_takeover_in_live_config_for_app(&AppType::Codex),
            "seeded codex live must be detected as taken-over"
        );

        // Model switch: same provider id, new model = "GLM-5.2".
...
"#
        ...
        assert_eq!(
            ...
            Some("GLM-5.2"),
            "live model must follow the GUI edit; got config.toml:\n{live_config}"
        );
        assert_eq!(
            parsed
                .get("model_providers")
            ...
            "local proxy base_url must survive the update"
        );
        let live_auth = std::fs::read_to_string(get_codex_auth_path()).expect("read auth.json");
        assert!(
            live_auth.contains("PROXY_MANAGED"),
            "auth.json must keep the proxy placeholder; got: {live_auth}"
        );
    }
...
impl ProviderService {
    ...
    pub fn update(
        ...
        if is_current {
            ...
            if should_sync_via_proxy {
                ...
                // Codex hot-switch on provider update: without this the live
                // `~/.codex/config.toml` is left with the previous provider's
                // `model = "..."` and `[model_providers.custom]` values, so
                // switching the model in the GUI while takeover is active has
                // no effect until the user toggles takeover off/on. Matches
                // the Claude branch above; also mirrors the Codex path in
                // `hot_switch_provider_inner` where `live_taken_over` triggers
                // this same helper. Gate on `live_taken_over` (not
                // `is_running`) so the live file is refreshed even when the
                // daemon is temporarily stopped — subsequent codex invocations
                // will otherwise still read the stale model.
    ...
    fn switch_normal(
        ...
        // Matpool seed 的 settings_config 在 DB 里 token 字段恒为空，真值在 OS keychain。
        // 切换写 live 之前注入一份带 token 的克隆，避免污染 DB；找不到 token 则原样写
        // （让用户切到 Matpool 但提示去 Token Wizard 填）。
        let injected =
            crate::services::matpool_inject::provider_with_injected_matpool_token(provider);
        let provider_to_write = injected.as_ref().unwrap_or(provider);
