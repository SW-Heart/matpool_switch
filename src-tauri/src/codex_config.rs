...
pub const MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID: &str = "custom";
pub const MATPOOL_SWITCH_CODEX_MODEL_CATALOG_FILENAME: &str = "matpool-switch-model-catalog.json";
...
pub fn get_codex_model_catalog_path() -> PathBuf {
    get_codex_config_dir().join(MATPOOL_SWITCH_CODEX_MODEL_CATALOG_FILENAME)
}
...
fn set_codex_model_catalog_json_field(
    ...
    match catalog_path {
        Some(path) => {
            // 使用绝对路径，避免 Codex 从当前工作目录解析相对路径时找不到文件
            // （`os error 2` 和 `Invalid configuration` 的直接原因）。
            ...
            let should_remove = doc
                ...
                .map(|path| {
                    Path::new(path).file_name().and_then(|name| name.to_str())
                        == Some(MATPOOL_SWITCH_CODEX_MODEL_CATALOG_FILENAME)
                })
                .unwrap_or(false);
            if should_remove {
...
/// matpool-switch–maintained catalog file referenced by `~/.codex/config.toml` and
...
/// matpool-switch–generated file (identified by filename
/// `matpool-switch-model-catalog.json`). A user-managed external catalog file is
...
pub fn read_codex_model_catalog_simplified_from_live() -> Result<Option<Value>, AppError> {
    ...
    let Some(catalog_path) = resolve_matpool_switch_catalog_path(&config_text, &generated_path)
    else {
        return Ok(None);
    };
...
/// Given `config.toml` text, resolve the on-disk path of the matpool-switch–owned
...
pub(crate) fn resolve_matpool_switch_catalog_path(
    config_text: &str,
    generated_path: &Path,
) -> Option<PathBuf> {
    ...
    let is_matpool_switch_owned = referenced_path.file_name().and_then(|name| name.to_str())
        == Some(MATPOOL_SWITCH_CODEX_MODEL_CATALOG_FILENAME);
    if !is_matpool_switch_owned {
        return None;
    }
...
pub fn inject_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    let existing_custom_conflicts = doc
        .get("model_providers")
        .and_then(|item| item.as_table())
        .and_then(|providers| providers.get(MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID))
        .and_then(|item| item.as_table())
    ...
    doc["model_provider"] = toml_edit::value(MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID);
    ...
    if let Some(providers) = doc["model_providers"].as_table_mut() {
        if !providers.contains_key(MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID) {
            providers.insert(
                MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID,
...
pub fn strip_codex_unified_session_bucket(config_text: &str) -> Result<String, AppError> {
    ...
    if doc.get("model_provider").and_then(|item| item.as_str())
        != Some(MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID)
    {
        return Ok(config_text.to_string());
    }
    ...
    let providers_empty = doc["model_providers"]
        .as_table_mut()
        .map(|providers| {
            providers.remove(MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID);
            providers.is_empty()
        })
        .unwrap_or(false);
    if providers_empty {
...
mod tests {
    ...
    fn unified_session_bucket_injects_for_empty_official_config() {
        ...
        let custom = doc["model_providers"][MATPOOL_SWITCH_CODEX_MODEL_PROVIDER_ID]
            .as_table()
    ...
    fn unified_session_bucket_preserves_other_keys_and_explicit_routing() {
        let with_catalog = "model_catalog_json = \"matpool-switch-model-catalog.json\"\n";
...
name = "any"
"#;
        let catalog_path = Path::new("/tmp/matpool-switch-model-catalog.json");
    ...
    fn resolve_catalog_path_returns_none_when_config_missing_field() {
        let generated = PathBuf::from("/tmp/.codex/matpool-switch-model-catalog.json");
        assert!(resolve_matpool_switch_catalog_path("", &generated).is_none());
        assert!(
            resolve_matpool_switch_catalog_path("model = \"gpt-5\"", &generated).is_none(),
            "no model_catalog_json field should yield None"
        );
    }

    #[test]
    fn resolve_catalog_path_accepts_matpool_switch_owned_file() {
        let generated = PathBuf::from("/tmp/.codex/matpool-switch-model-catalog.json");
        let config = r#"model_catalog_json = "/tmp/.codex/matpool-switch-model-catalog.json"
"#;
        let resolved =
            resolve_matpool_switch_catalog_path(config, &generated).expect("path resolves");
...
"#;
        assert!(
            resolve_matpool_switch_catalog_path(config, &generated).is_none(),
...
model = "glm-5"
"#;
        // Simulate a WSL UNC path as matpool-switch would see it on Windows;
        // the function now writes just the relative filename.
        let unc_path =
            Path::new(r"\\wsl.localhost\Ubuntu\home\user\.codex\matpool-switch-model-catalog.json");
        ...
        assert_eq!(
            written_path, MATPOOL_SWITCH_CODEX_MODEL_CATALOG_FILENAME,
...
model = "glm-5"
"#;
        let regular_path = Path::new("/home/user/.codex/matpool-switch-model-catalog.json");
        ...
        assert_eq!(
            ...
            regular_path.to_str(),
            "should write the full path"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_matpool_switch_owned_by_filename() {
        ...
        let input = r#"model_catalog_json = "/home/user/.codex/matpool-switch-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove matpool-switch-owned field regardless of path format"
        );
    }
...
model_catalog_json = "matpool-switch-model-catalog.json"
"#;
        let generated_path = PathBuf::from("/home/user/.codex/matpool-switch-model-catalog.json");
        let result = resolve_matpool_switch_catalog_path(config_text, &generated_path);
        assert_eq!(
            result,
            Some(generated_path),
...
"#;
        ...
        assert_eq!(
            result, None,
            "user-owned catalog should not be claimed by matpool-switch"
        );
    }

    #[test]
    fn set_catalog_json_none_removes_relative_path() {
        let input = r#"model_catalog_json = "matpool-switch-model-catalog.json"
"#;
        let result = set_codex_model_catalog_json_field(input, None).unwrap();
        let parsed: toml::Value = toml::from_str(&result).unwrap();
        assert!(
            parsed.get("model_catalog_json").is_none(),
            "None arm should remove relative matpool-switch-owned field"
        );
    }
}
