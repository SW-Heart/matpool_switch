...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/farion1231/matpool-switch/releases/latest",
            None::<String>,
        )
...
pub async fn get_tool_versions(
    ...
    for tool in requested {
        ...
        futs.push(get_single_tool_version_impl(
            tool,
            tool_wsl_shell,
            tool_wsl_shell_flag,
        ));
...
async fn get_single_tool_version_impl(
    ...
    //
    // 加 2s 超时保护：若 npm/pypi 不可达（用户在内网 / 防火墙 / 代理配置异常 / 我们
    // 自己的 proxy 还在初始化导致 HTTP 客户端阻塞），主页 detection 不应被卡 5 分钟。
    // 拿不到 latest 不致命——前端只是少显示一个"有更新"提示，本地 version 已经够用。
    ...
    let latest_version =
        match tokio::time::timeout(std::time::Duration::from_secs(2), latest_fut).await {
            Ok(v) => v,
            Err(_) => {
                log::warn!("fetch_npm_latest_for_tool timeout for {tool} (>2s); skipping");
                None
            }
        };
...
async fn fetch_github_latest_version(client: &reqwest::Client, repo: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .header("User-Agent", "matpool-switch")
...
///
/// 判定顺序（命中即返回）：
/// ① Hermes → `<bin_path 绝对> update`;Hermes CLI 自己知道安装环境,避免 matpool-switch
...
fn launch_macos_terminal(config_file: &std::path::Path, cwd: Option<&Path>) -> Result<(), String> {
    ...
    let temp_dir = std::env::temp_dir();
    let script_file = temp_dir.join(format!("matpool_switch_launcher_{}.sh", std::process::id()));
...
fn launch_windows_terminal(
    ...
    let bat_file = temp_dir.join(format!("matpool_switch_claude_{}.bat", std::process::id()));
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("matpool_switch_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/bin/bash
trap 'rm -f "{script_path}"' EXIT
echo "[matpool-switch] Starting: {label}"
echo ""
{cmd}
echo ""
echo "[matpool-switch] Command exited. Press any key to close."
read -n 1 -s
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("matpool_switch_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [matpool-switch] Starting: {label}\r\necho.\r\n{cmd}\r\necho.\r\necho [matpool-switch] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            label = label,
            cmd = command_line,
...
mod tests {
    ...
    mod anchored_upgrade {
        ...
        fn hermes_uses_cli_update_anchor() {
            // Hermes 自带 `hermes update`;锚定到命令行默认那处 CLI,避免 matpool-switch 猜
    ...
    mod install_strategy {
        ...
        fn hermes_install_uses_official_installer() {
            // Hermes 官方 installer 会处理 Python 3.11+/uv 等运行时;不要再从 matpool-switch
    ...
    fn resolve_launch_cwd_rejects_missing_directory() {
        let unique = std::time::SystemTime::now()
            ...
            .as_nanos();
        let missing = std::env::temp_dir().join(format!("matpool-switch-missing-{unique}"));
    ...
    fn iterm2_applescript_cold_start_avoids_current_window_before_one_exists() {
        let script = build_macos_iterm2_applescript(Path::new("/tmp/matpool_switch_launcher.sh"));
    ...
    fn terminal_applescript_cold_start_uses_launch_before_do_script() {
        let script = build_macos_terminal_applescript(Path::new("/tmp/matpool_switch_launcher.sh"));
    ...
    fn ghostty_applescript_cold_start_uses_initial_command() {
        let script = build_macos_ghostty_applescript(Path::new("/tmp/matpool_switch_launcher.sh"));

        // Warm launches execute through the AppleScript command property, not `open -na ... -e`.
        assert!(
            script.contains(r#"set launcher_command to "bash '/tmp/matpool_switch_launcher.sh'""#),
    ...
    fn dash_c_command_wraps_script_path_inside_quoted_arg() {
        ...
        let s = build_macos_dash_c_command(Path::new("/tmp/matpool_switch_launcher_1.sh"));
        assert_eq!(s, "exec bash '/tmp/matpool_switch_launcher_1.sh'");
