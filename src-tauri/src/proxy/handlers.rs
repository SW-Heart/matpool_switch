...
pub async fn handle_models(State(state): State<ProxyState>) -> Result<Json<Value>, ProxyError> {
    ...
    let active_catalog_path = match crate::codex_config::read_codex_config_text() {
        Ok(config_text) => {
            crate::codex_config::resolve_matpool_switch_catalog_path(&config_text, &generated_path)
        }
        Err(_) => None,
    };
    ...
    // 无目录文件时返回 Matpool 常见模型列表，确保 Claude Code 等工具也能列举模型
    log::debug!("[models] no Codex catalog found; returning built-in Matpool model list");
    Ok(Json(json!({
        "models": [
            {"model": "claude-opus-4-8", "display_name": "Claude Opus 4.8"},
            {"model": "claude-sonnet-4-6", "display_name": "Claude Sonnet 4.6"},
            {"model": "claude-haiku-4-5", "display_name": "Claude Haiku 4.5"},
            ...
            {"model": "gemini-2.5-pro", "display_name": "Gemini 2.5 Pro"},
            {"model": "Doubao-Seed-2.0-Code-Preview", "display_name": "Doubao Seed Code"},
            {"model": "Qwen3-Coder-Flash", "display_name": "Qwen Coder Flash"},
            {"model": "DeepSeek-Chat", "display_name": "DeepSeek Chat"},
            {"model": "Kimi-K2.5", "display_name": "Kimi K2.5"},
            {"model": "Hy3-Preview", "display_name": "Hy3 Preview"}
        ]
    })))
}
...
pub async fn handle_responses(
    ...
    let req_model = body
        ...
        .unwrap_or("unknown");
    log::debug!(
        "[Codex/Responses] model={req_model}, stream={is_stream}, tool_calls={tc}",
        tc = codex_tool_context.chat_tools().len(),
    );
    ...
    log::debug!(
        "[Codex/Responses] decision: convert={do_convert}, provider={}, model={}",
        ctx.provider.id,
        ctx.request_model,
    );
    if do_convert {
...
pub async fn handle_responses_compact(
    ...
    let req_model = body
        ...
        .unwrap_or("unknown");
    log::debug!(
        "[Codex/ResponsesCompact] model={req_model}, stream={is_stream}, tool_calls={tc}",
        tc = codex_tool_context.chat_tools().len(),
    );
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        // 避免「以为是 Matpool Switch 封装了 nginx / 是本地代理的锅」这种反复出现的误解。
        format!(
            concat!(
                ...
                "provider's server-side limit, not a Matpool Switch limit. ",
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "Matpool Switch local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
fn codex_proxy_error_code(error: &ProxyError) -> &'static str {
    match error {
        ProxyError::ForwardFailed(_) => "matpool_switch_forward_failed",
        ProxyError::Timeout(_) | ProxyError::StreamIdleTimeout(_) => "matpool_switch_timeout",
        ProxyError::NoAvailableProvider => "matpool_switch_no_available_provider",
        ProxyError::AllProvidersCircuitOpen => "matpool_switch_all_providers_circuit_open",
        ProxyError::NoProvidersConfigured => "matpool_switch_no_providers_configured",
        ProxyError::MaxRetriesExceeded => "matpool_switch_max_retries_exceeded",
        ProxyError::ProviderUnhealthy(_) => "matpool_switch_provider_unhealthy",
        ProxyError::ConfigError(_) => "matpool_switch_config_error",
        ProxyError::TransformError(_) => "matpool_switch_transform_error",
        ProxyError::InvalidRequest(_) => "matpool_switch_invalid_request",
        ProxyError::AuthError(_) => "matpool_switch_auth_error",
        ProxyError::UpstreamError { .. } => "matpool_switch_upstream_error",
        ProxyError::DatabaseError(_) => "matpool_switch_database_error",
        ProxyError::Internal(_) => "matpool_switch_internal_error",
        ProxyError::AlreadyRunning
        | ProxyError::NotRunning
        | ProxyError::BindFailed(_)
        | ProxyError::StopTimeout
        | ProxyError::StopFailed(_) => "matpool_switch_proxy_error",
    }
}
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("Matpool Switch local proxy failed"));
        ...
        assert_eq!(body["error"]["code"], "matpool_switch_forward_failed");
