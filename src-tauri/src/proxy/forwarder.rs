...
#[cfg(not(feature = "desktop"))]
static HEADLESS_CODEX_AUTH: OnceLock<Arc<RwLock<CodexOAuthManager>>> = OnceLock::new();

#[cfg(not(feature = "desktop"))]
fn headless_copilot_auth() -> Arc<RwLock<CopilotAuthManager>> {
    HEADLESS_COPILOT_AUTH
        .get_or_init(|| {
...
#[cfg(not(feature = "desktop"))]
fn headless_codex_auth() -> Arc<RwLock<CodexOAuthManager>> {
    HEADLESS_CODEX_AUTH
        .get_or_init(|| {
            Arc::new(RwLock::new(CodexOAuthManager::new(
                crate::config::get_app_config_dir(),
            )))
        })
        .clone()
...
pub struct RequestForwarder {
    ...
    app_handle: Option<DesktopAppHandle>,
    #[cfg(not(feature = "desktop"))]
    copilot_auth: Arc<RwLock<CopilotAuthManager>>,
    #[cfg(not(feature = "desktop"))]
    codex_auth: Arc<RwLock<CodexOAuthManager>>,
...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        if is_copilot && !is_full_url {
            #[cfg(not(feature = "desktop"))]
            {
                let copilot_auth = self.copilot_auth.read().await;
                ...
                if dynamic_endpoint != base_url {
                    log::debug!(
                        "[Copilot] 使用 CLI 动态 API endpoint: {} (原: {})",
                        dynamic_endpoint,
                        base_url
                    );
        ...
        let mut auth_headers = if let Some(mut auth) = adapter.extract_auth(provider) {
            ...
            if auth.strategy == AuthStrategy::GitHubCopilot {
                #[cfg(not(feature = "desktop"))]
                {
                    ...
                    let token_result = match &account_id {
                        Some(id) => {
                            log::debug!("[Copilot] CLI 使用指定账号 {id} 获取 token");
                            copilot_auth.get_valid_token_for_account(id).await
                        }
                        None => {
                            log::debug!("[Copilot] CLI 使用默认账号获取 token");
                    ...
                    match token_result {
                        Ok(token) => {
                            ...
                            log::debug!(
                                "[Copilot] CLI 成功获取 Copilot token (account={})",
                                account_id.as_deref().unwrap_or("default")
                            );
                        }
                        Err(e) => {
                            log::error!(
                                "[Copilot] CLI 获取 Copilot token 失败 (account={}): {e}",
                                account_id.as_deref().unwrap_or("default")
                            );
            ...
            if auth.strategy == AuthStrategy::CodexOAuth {
                #[cfg(not(feature = "desktop"))]
                {
                    let codex_auth = self.codex_auth.read().await;
                    ...
                    let token_result = match &account_id {
                        Some(id) => {
                            log::debug!("[CodexOAuth] CLI 使用指定账号 {id} 获取 token");
                            codex_auth.get_valid_token_for_account(id).await
                        }
                        None => {
                            log::debug!("[CodexOAuth] CLI 使用默认账号获取 token");
                    ...
                    match token_result {
                        Ok(token) => {
                            ...
                            log::debug!(
                                "[CodexOAuth] CLI 成功获取 access_token (account={})",
                                codex_oauth_account_id.as_deref().unwrap_or("default")
                            );
                        }
                        Err(e) => {
                            log::error!("[CodexOAuth] CLI 获取 access_token 失败: {e}");
                            return Err(ProxyError::AuthError(format!(
                                "Codex OAuth 认证失败: {e}"
                            )));
                        }
