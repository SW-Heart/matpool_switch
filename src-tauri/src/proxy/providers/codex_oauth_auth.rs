...
/// User-Agent
const CODEX_USER_AGENT: &str = "matpool-switch-codex-oauth";
...
impl CodexOAuthManager {
    ...
    fn write_store_atomic(&self, content: &str) -> Result<(), CodexOAuthError> {
        ...
        let write_result = (|| -> Result<(), CodexOAuthError> {
            #[cfg(unix)]
            {
...
mod tests {
    ...
    fn test_write_store_atomic_removes_temp_file_when_replace_fails() {
        ...
        let leaked_tmp = std::fs::read_dir(temp.path())
            .unwrap()
            ...
            .any(|entry| {
                entry
                    .file_name()
                    .to_string_lossy()
                    .starts_with("codex_oauth_auth.json.tmp.")
            });
        assert!(!leaked_tmp);
    }
