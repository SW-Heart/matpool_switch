...
pub fn should_convert_codex_responses_to_chat(provider: &Provider, endpoint: &str) -> bool {
    ...
    // 1. provider 元数据明确要求 chat 转换（用户前端选模型时写入）
    if codex_provider_uses_chat_completions(provider) {
        return true;
    }

    // 2. Matpool seed 默认也走 chat 转换——其 NewAPI 网关只保证 Chat 兼容，
    //    /v1/responses 仅对 GPT-5.5 等极少模型有效。provider.api_format 可能
    //    因 takeover 同步丢失（用户之前遇到的"中断问题"），但 provider.id 始终稳定。
    if provider.id.starts_with("matpool-") {
        return true;
    }
