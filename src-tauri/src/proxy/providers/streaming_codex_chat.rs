...
impl Default for ChatToResponsesState {
    fn default() -> Self {
        Self {
            ...
            completed: false,
            response_id: "resp_matpoolswitch".to_string(),
            model: String::new(),
            created_at: 0,
            next_output_index: 0,
...
impl ChatToResponsesState {
    ...
    fn push_tool_call_delta(&mut self, tool_call: &Value, reasoning: Option<&str>) -> Vec<Bytes> {
        ...
        // Some Chat providers (e.g. Kimi/Moonshot on `wire_api=responses` → chat)
        // resend `id`/`function.name` as empty strings in continuation chunks
        // after the first chunk provided the real values. Treat empty strings as
        // absent deltas so we never overwrite a previously-captured id/name with
        // `""` — otherwise the finalized `function_call` reaches Codex with
        // empty name/call_id and Codex reports `unsupported call`, which in the
        // next turn produces a dangling assistant tool_call that upstream
        // (Moonshot/Kimi) rejects with HTTP 400 "tool_call_ids ... : :0 did not
    ...
    fn finalize(&mut self) -> Vec<Bytes> {
        ...
        log::debug!(
            "[CodexStream] finalize: status={status}, items={n}, finish_reason={fr:?}",
...
mod tests {
    ...
    /// Regression: Kimi/Moonshot (and some other Chat providers) send the
    /// tool_call `id` and `function.name` only in the first delta chunk;
    /// subsequent chunks resend them as EMPTY STRINGS while continuing to
    /// stream `arguments`. Empty strings must NOT overwrite the previously
    /// captured id/name — otherwise the finalized Responses `function_call`
    /// reaches Codex with empty name and Codex reports `unsupported call`,
    /// which produces a dangling assistant `tool_call` in the next turn's
    /// history and the upstream rejects with 400
    /// ("tool_call_ids ... : :0 did not have response messages").
    #[tokio::test]
    async fn preserves_first_chunk_id_and_name_when_continuation_chunks_send_empty_strings() {
        let output = collect(vec![
            "data: {\"id\":\"chatcmpl_kimi\",\"model\":\"Kimi-K2.7-Code\",\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"type\":\"function\",\"id\":\"shell_command_0\",\"function\":{\"name\":\"shell_command\"}}]}}]}\n\n",
            "data: {\"id\":\"chatcmpl_kimi\",\"model\":\"Kimi-K2.7-Code\",\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"\",\"function\":{\"name\":\"\",\"arguments\":\"{\\\"\"}}]}}]}\n\n",
            "data: {\"id\":\"chatcmpl_kimi\",\"model\":\"Kimi-K2.7-Code\",\"choices\":[{\"delta\":{\"tool_calls\":[{\"index\":0,\"id\":\"\",\"function\":{\"name\":\"\",\"arguments\":\"command\\\":\\\"echo hi\\\"}\"}}]},\"finish_reason\":\"tool_calls\"}]}\n\n",
            "data: [DONE]\n\n",
        ])
        .await;

        assert!(
            output.contains("\"call_id\":\"shell_command_0\""),
            "call_id from first chunk must survive empty continuation deltas; got:\n{output}"
        );
        assert!(
            output.contains("\"name\":\"shell_command\""),
            "function name from first chunk must survive empty continuation deltas; got:\n{output}"
        );
        assert!(
            !output.contains("\"call_id\":\"\""),
            "no empty call_id should leak into the emitted Responses items; got:\n{output}"
        );
        assert!(output.contains("event: response.function_call_arguments.done"));
    }
}
