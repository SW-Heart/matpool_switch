...
//!
//! This module is used when the Codex client talks to Matpool Switch through the
...
fn chat_tool_call_to_response_item(
    ...
    // Fallback to "unknown_tool" when the upstream omits a function name for a
    // tool_call. Emitting an empty `name` in the Responses `function_call` item
    // causes Codex to raise `unsupported call`, which then leaves a dangling
    // assistant `tool_call` in the next turn's history and upstream (e.g.
    // Moonshot/Kimi) rejects with 400 ("tool_call_ids ... : :0 did not have
    // response messages"). The streaming path enforces the same fallback.
    let name = function
        .get("name")
        .and_then(|v| v.as_str())
        .filter(|value| !value.is_empty())
...
pub(crate) fn response_id_from_chat_id(id: Option<&str>) -> String {
    let id = id.unwrap_or("matpoolswitch");
...
mod tests {
    ...
    // https://github.com/farion1231/matpool-switch/issues/3557
