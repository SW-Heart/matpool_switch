...
const MATPOOL_SWITCH_SQL_EXPORT_HEADER: &str = "-- Matpool Switch SQLite 导出";
...
impl Database {
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_matpool_switch_sql_export(sql_content)?;
    ...
    fn validate_matpool_switch_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(MATPOOL_SWITCH_SQL_EXPORT_HEADER)
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 Matpool Switch 导出的 SQL 备份文件。",
            "Only SQL backups exported by Matpool Switch are supported.",
        ))
    }
...
mod tests {
    ...
    fn periodic_maintenance_runs_even_when_auto_backup_disabled() -> Result<(), AppError> {
        ...
        let test_home =
            std::env::temp_dir().join("matpool-switch-periodic-maintenance-backup-disabled-test");
        let _ = std::fs::remove_dir_all(&test_home);
        std::fs::create_dir_all(&test_home).expect("create test home");
        std::env::set_var("MATPOOL_SWITCH_TEST_HOME", &test_home);
